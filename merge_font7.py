...
for gname, newname in name_map.items():
    ...
    for op, args in round_coords(rec):
        getattr(pen, op)(*args)
    glyph = pen.glyph()
    src_glyf[newname] = glyph
    # 关键修复：lsb 必须 = 字形真实 xMin（freetype/Android 按 lsb 定位字形绘制起点）
    # 统一 LSB=50 会把窄字形推到格子最左边（"左对齐"的根源！）
    from fontTools.pens.boundsPen import BoundsPen
    bp = BoundsPen(src_glyf)
    glyph.draw(bp, src_glyf)
    real_lsb = int(bp.bounds[0]) if bp.bounds else LSB
    hmtx[newname] = (NEW_ADVANCE, real_lsb)
...
out = "SourceCodeProNerdMono-CJK7.ttf"
src.save(out)
